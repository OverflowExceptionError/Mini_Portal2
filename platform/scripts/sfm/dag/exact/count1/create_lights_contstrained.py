...
# Create a child dag of the light kit root that may be rotated
lightHandleDag = sfm.CreateDag( "lightHandle", animSet=lightKitAnimSet )
sfmUtils.Parent( lightHandleDag, lightRoot, bMaintainWorldPos=False )
...
# Create the lights
CreateLight( lightKitAnimSet, rootGroup, colorizeConn, lightHandleDag, "keyLight",     0,   45,  -25, intensity = 150, castsShadows = True, color = vs.Color( 227, 149, 150 ), volumetric = True, farZAtten = 300, radius=7 )
CreateLight( lightKitAnimSet, rootGroup, colorizeConn, lightHandleDag, "bounceLight",  0,  -70,  -120, intensity = 12, castsShadows = True, color = vs.Color( 150, 227, 227 ) )
CreateLight( lightKitAnimSet, rootGroup, colorizeConn, lightHandleDag, "fillLight",    0,  -12,  -135, intensity = 10, castsShadows = True, color = vs.Color( 255, 255, 255 ), farZAtten = 150 )
CreateLight( lightKitAnimSet, rootGroup, colorizeConn, lightHandleDag, "rimLight",     0,   15,    60, intensity = 700, castsShadows = True, farZAtten = 150 )


#Hide contraint weights
targetAnimSet = sfm.GetCurrentAnimationSet()
rootTargetGroup = targetAnimSet.GetRootControlGroup()
targetContraintGroup= rootTargetGroup.FindChildByName( "aimConstraint_lightRoot", True)
rootGroup.AddChild ( targetContraintGroup )
targetContraintGroup= rootTargetGroup.FindChildByName( "pointConstraint_lightRoot", True)
rootGroup.AddChild ( targetContraintGroup )
